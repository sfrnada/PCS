package com.example.appsafira.data.local

import android.content.Context
import android.content.SharedPreferences
import com.example.appsafira.data.model.ActionState
import com.example.appsafira.data.model.AuthUser
import com.example.appsafira.util.getObject
import com.example.appsafira.util.putObject
...
class AuthPref(val  context:Context) {
    ...
    suspend fun login(email: String, password: String): ActionState<AuthUser> {
        ...
        return if (user == null){
            ...
            ActionState(message = "Email dan Password tidak bolek kosong", isSuccess = false)
        } else if (user.email == email && user.password == password) {
            isLogin = true
            ActionState(authUser, message = "Anda berhasil login")
        } else {
            ActionState(message = "Emsil atau Password salah", isSuccess = false)
        }
    }
